...
# fname='Sydney--Australia'
landmark_dir = './landmarks/landmark-Los-Angeles--CA.txt'
house_dir = './houses/houseinfo.txt'
...
def draw_land_ap(fname, num, fig):
    ...
    filename_landmark = landmark_dir
    filename = house_dir
    landmark_lat = []
    landmark_lon = []
    house_lat = []
    ...
    filwname = './result_ap/landmark_LosAngeles.txt'
    ...
    for i in range(len(idx)):
        ...
        if rad >= radius[int(idx[i][0]) - 1]:
            ...
            #                 radius[int(idx[i_co][0]) - 1], fig, co, num_1, num_2, num)
            # plt.xticks(fontproperties=font)
            # plt.yticks(fontproperties=font)


def draw_house_ap(fname, num):
    filename = house_dir
    inf_train, prices, inf_test, price_test = price_infrustration(filename)
    inf_train_huff = []
    inf_test_huff = []
    x1 = []
    x2 = []
    y1 = []
    y2 = []
    for i_1 in range(len(inf_train)):
        a = Huffman_coding(inf_train[i_1][0:15])
        b = Huffman_coding(inf_train[i_1][15:30])
        x1.append(a)
        y1.append(b)
        inf_train_huff.append([a, b])

    dis_all = []
    k = 0
    n = len(inf_train_huff)
    m = n * n - n
    s = eng.zeros(m, 3)
    for i in range(len(inf_train_huff)):
        for j in range(len(inf_train_huff)):
            if j != i:
                dis = distance(inf_train_huff[i], inf_train_huff[j])
                dis_all.append(dis)
                s[k][0] = i + 1
                s[k][1] = j + 1
                s[k][2] = dis
    ...
    p = matlab.double([p])
    idx, netsim, dpsim, expref = eng.apclusterSparse(s, p, nargout=4)
    # idx,netsim,dpsim,expref=eng.eval("apclusterSparse(%d,%f)"%(s,p),nargout = 4)
    idx = list(idx)

    filwname = './result_ap/house_LosAngele.txt'
    filw = open(filwname, 'a')
    print len(idx), len(inf_train_huff), fname
    plt.subplot(1, 1, num)
    ...
    for i in range(len(idx)):
        col = colors[(int(idx[i][0]) - 1) % len(colors)]
        # crll=crl[(int(idx[i][0])-1)%6]
        plt.plot(inf_train_huff[i][0], inf_train_huff[i][1], markerfacecolor=col, marker='.')
        plt.plot(inf_train_huff[int(idx[i][0]) - 1][0], inf_train_huff[int(idx[i][0]) - 1][1], markerfacecolor=col,
                 marker='o')
        plt.plot((inf_train_huff[i][0], inf_train_huff[int(idx[i][0]) - 1][0]),
                 (inf_train_huff[i][1], inf_train_huff[int(idx[i][0]) - 1][1]), col)
        plt.title(fname, fontproperties=font)
        plt.xlabel("facility-varity", fontproperties=font)
        plt.ylabel("facility-varity", fontproperties=font)

    # filw.write(str(inf_train_huff[i][0])+'\t')
    # filw.write(str(inf_train_huff[i][1])+'\t')
    # filw.write(str(prices[i])+'\t')
    # filw.write(str(inf_train_huff[int(idx[i][0])-1][0])+'\t')
    # filw.write(str(inf_train_huff[int(idx[i][0])-1][1])+'\t')
    # filw.write(str(prices[int(idx[i][0])-1])+'\n')
    plt.xticks([0, 3000, 6000, 9000, 12000, 15000], fontproperties=font)
    plt.yticks(fontproperties=font)

    plt.axes([0.5, 0.5, .3, .3])
    font = FontProperties(family='Times New Roman', size=14, weight='normal', style='normal')
    colors = ['#293047', '#473c8b', '#007947', '#8f4b38', '#FF0000', "#CD2626", "#9932CC", "#8B1A1A", "#0A0A0A",
              "#0000AA", "#008B00", "#9Aff9a", "#CDAD00", "#FF8c00", "#ff1493", "#FFFF00", "#EE00EE", "#bf3eff"]
    for i in range(len(idx)):
...
def plot_infrust_cluster(idx, landmarks, fname, col):
    font = FontProperties(family='Times New Roman', size=14, weight='normal', style='normal')
    colors = ['#293047', '#473c8b', '#007947', '#8f4b38', '#FF0000', "#CD2626", "#9932CC", "#8B1A1A", "#0A0A0A",
              "#0000AA", "#008B00", "#9Aff9a", "#CDAD00", "#FF8c00", "#ff1493", "#FFFF00", "#EE00EE", "#bf3eff"]
    # radius=[0]*len(landmarks)
    ...
    for i in range(len(idx)):
        col = colors[(int(idx[i]) - 1) % len(colors)]
        plt.plot(landmarks[i][0], landmarks[i][1], markerfacecolor=col, marker='.')
        plt.plot(landmarks[int(idx[i]) - 1][0], landmarks[int(idx[i]) - 1][1], markerfacecolor=col, marker='o')
        plt.plot((landmarks[i][0], landmarks[int(idx[i]) - 1][0]), (landmarks[i][1], landmarks[int(idx[i]) - 1][1]),
                 col)
        # plt.title(fname,fontproperties=font)
        plt.xlabel("Latitude", fontproperties=font)
        plt.ylabel("Longitude", fontproperties=font)
    # rad=math.sqrt(sum((landmarks[i][0]-landmarks[int(idx[i][0])-1][0])**2+(landmarks[i][1]-landmarks[int(idx[i][0])-1][1])**2))

    plt.xticks(fontproperties=font)
    plt.yticks(fontproperties=font)


def plot_house_point(idx, landmarks, fname):
    font = FontProperties(family='Times New Roman', size=14, weight='normal', style='normal')
    colors = ['#293047', '#473c8b', '#007947', '#8f4b38', '#FF0000', "#CD2626", "#9932CC", "#8B1A1A", "#0A0A0A",
              "#0000AA", "#008B00", "#9Aff9a", "#CDAD00", "#FF8c00", "#ff1493", "#FFFF00", "#EE00EE", "#bf3eff"]
    # radius=[0]*len(landmarks)
    ...
    for i in range(len(idx)):
        col = colors[(int(idx[i]) - 1) % len(colors)]
        plt.plot(landmarks[i][0], landmarks[i][1], 'g.')
        # plt.plot(landmarks[int(idx[i])-1][0],landmarks[int(idx[i])-1][1],markerfacecolor=col,marker='o')
        # plt.plot((landmarks[i][0],landmarks[int(idx[i])-1][0]),(landmarks[i][1],landmarks[int(idx[i])-1][1]),col)
        # plt.title(fname,fontproperties=font)
        plt.xlabel("Latitude", fontproperties=font)
        plt.ylabel("Longitude", fontproperties=font)
    # rad=math.sqrt(sum((landmarks[i][0]-landmarks[int(idx[i][0])-1][0])**2+(landmarks[i][1]-landmarks[int(idx[i][0])-1][1])**2))
...
#
#
# def find_review_information(roomid):
#     url = 'https://zh.airbnb.com/rooms/' + roomid
#     driver.get(url)
#     # data = driver.title
#     # print data.decode('utf-8')
#     # assert "airbnb" in driver.title
#     # try:
#     #	 rates=driver.find_elements_by_xpath("//strong[@class='text-beach']")
#     #	 ratesa=driver.find_elements_by_class("decimal-star-rating-wrapper")
#     #	 print ratesa
#     #	 for num in rates:
#     #		 print num.text
#     #	 rate= rates[1].text
...
#     #	 rate='0'
#     try:
#         num = driver.find_elements_by_xpath("//span[@class='badge-pill-count']")
#         if len(num) == 2:
#             review = num[0].text
#             recomm = num[1].text
#         elif len(num) == 1:
...
#         review = '0'
#         recomm = '0'
#     return review, recomm
#
#
# # a='7558596'
# # find_review_information(a)
def test(fname, fig, num_1, num_2, num):
    # from statics import infrustration
    # filename='../data/landmark-'+fname+'.txt'
    font = FontProperties(family='Times New Roman', size=14, weight='normal', style='normal')
    filename_landmark = landmark_dir
    filename_house = house_dir
    ...
    house_value = []
    dis_all = []
    for j in open(filename_house, 'r').readlines():
    ...
    filwname = './result_ap/house_LosAngeles.txt'
    filw = open(filwname, 'a')
    ...
    filw_house_cluster = open('./result_ap/cluster2_house_LosAngeles.txt', 'a')
    ...
    for hi in range(len(houses)):
        ...
        try:
            ...
            filw_house_cluster.write(
                str(landmark_lat[int(idx[hosue_dis2lanM[hi].index(min(hosue_dis2lanM[hi]))][0]) - 1]) + '\t')
            filw_house_cluster.write(
                str(landmark_lon[int(idx[hosue_dis2lanM[hi].index(min(hosue_dis2lanM[hi]))][0]) - 1]) + '\t')
    ...
    infrust, house_price = infrustration(filename_house)
    ...
    filw_house_facility_cluster = open('./result_ap/facility7_cluster_LosAngeles.csv', 'a')
    kkk = 0
...
#     tu_2_tu()
#
# plot_circle(i, 4, 1, fig)
# plt.show()
# name = []
# for na in open('../locations_paper', 'r').readlines():
#     name.append(na.strip())
name = ['Los-Angeles--CA']
for i in range(len(name)):
    # draw_house_ap(name[i], 1)  # Without geo information
    # draw_land_ap(name[i], i + 1, fig)
