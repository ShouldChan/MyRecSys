...
def read_training_data():
    ...
    train_data_file = open('./data_train.txt', 'r')
...
def get_locations():
    locations = {}
    # print 'dafas'
    with open('./data_v1.txt', 'r') as fread:
        lines = fread.readlines()
        for line in lines:
            temp = line.strip().split('\t')
            l = int(temp[1]) - 1
            locations[l] = [float(temp[2]), float(temp[3])]
            # print l, locations[l]
    return locations
    '''
    train_data_file = open(data_dir + dataset + "train" + file_suffix, 'r')
    for eachline in train_data_file:
        raw_data = eachline.strip().split('\t')
        lc, li = int(raw_data[1]) - 1, int(raw_data[5]) - 1
        locations[lc] = [float(raw_data[2]), float(raw_data[3])]
        locations[li] = [float(raw_data[6]), float(raw_data[7])]
        #print "train_get_location:",locations[lc],"\t",locations[li]

        #将test里的poi读进去 构成一个整体的poi
    test_data_file = open(data_dir + dataset + "test" + file_suffix, 'r')
    for eachline in test_data_file:
        raw_data = eachline.strip().split('\t')
        lc, li = int(raw_data[1]) - 1, int(raw_data[5]) - 1
        locations[lc] = [float(raw_data[2]), float(raw_data[3])]
        locations[li] = [float(raw_data[6]), float(raw_data[7])]
        #lc_lat, lc_lon = float(raw_data[2]), float(raw_data[3])
        #li_lat, li_lon = float(raw_data[6]), float(raw_data[7])
        #locations.add((lc, lc_lat, lc_lon))
        #locations.add((li, li_lat, li_lon))
        #print "test_get_location:",locations[lc],"\t",locations[li]
    return locations
    '''
...
def learning(train_data, visits, locations, cont):
    ...
    try:
        ...
        np.savetxt(model_dir + "UP.txt", UP)
        np.save(model_dir + "LP", LP)
        np.savetxt(model_dir + "LP.txt", LP)
        np.save(model_dir + "LS", LS)
        np.savetxt(model_dir + "LS.txt", LS)
        print "Model saved..."
import operator
